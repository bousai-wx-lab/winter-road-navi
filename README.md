...
一年366日の日最低気温平年値と全国の高速道路・一般道路を重ねて確認する、Bousai Wx Labの公開ツールです。

公開URL: https://bousai-wx-lab.github.io/winter-road-navi/

## 現在の公開範囲

気象庁の1991〜2020年のアメダス日別平年値から、全国に偏りなく広がる180地点を選び、日最低気温の参考分布として表示します。1月1日から12月31日まで（2月29日を含む366日）をスライダーで移動でき、前日・翌日ボタンと、1周30秒・15秒・8秒の自動再生を備えています。色階級は年間を通して固定しているため、季節変化を同じ基準で比較できます。

気温分布は観測地点の値をぼかして面状に見せる本ツール独自の加工表示です。気象庁の公式メッシュ平年値そのものではなく、予報・実況・路面温度・凍結判定でもありません。0℃の色は比較の目安であり、安全判断には使えません。
...
地図の移動、拡大・縮小、気温分布の表示・濃さ、道路種別の表示切替、全国表示への復帰、道路のクリック確認ができます。キーボードでは左右キーで1日ずつ移動し、スペースキーで再生・停止できます。
...
積雪、凍結、通行規制、渋滞、経路案内は表示しません。現在の道路状況や安全を示す地図ではありません。

## データと表示

- 気温：気象庁「2020年平年値（第5版）アメダス日別平年値」の日最低気温（統計期間1991〜2020年）
- 地点：日最低気温が366日そろう地点から、緯度経度上の最遠点サンプリングで180地点を決定
- 数値：気象庁の0.1℃単位の値を変更せず、ブラウザ向けに符号付き16bit整数へ格納
- 表示：固定色階級の円を重ねてぼかした参考分布。地点間の値を計算で推定するものではない
...
気温データは気象庁ホームページの利用規約に従い、出典と加工内容を表示しています。

- https://www.data.jma.go.jp/obd/stats/data/mdrr/normal/index.html
- https://www.jma.go.jp/jma/kishou/info/coment.html

`scripts/build_temperature_data.py` は、気象庁の公開ZIPを取得して同じ選抜規則から `temperature-data.js` を再生成します。生成物には取得元ZIPのSHA-256を記録しています。
